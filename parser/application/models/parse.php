...
class Parse extends CI_Model {
  ...
  {              
    ...
    foreach ($logEntry_lines as $label => $value) {
      ...
      switch ($label) {
        ...
        preg_match('/^[0-9A-F]{4}/', $data_array[1]) ? $fields['battery_level'] = hexdec($data_array[1]) : $this->_setErrorMsg('battery',$fields);
        break;
      } 
    }
    return $fields; 
  ...
  // GETS
	...
	function getActiveSensor($sensorid)
	{
		$this->db->where('sensor_id', $sensorid);		
		$this->db->where('sensor_inuse', 1);    
